...
function M.build(opts)
    ...
    -- Колонка: выдаём готовый объект + модуль audio/speaker через znatokos.use.
    if hasCap(caps, "periph.speaker") then
        -- Прямой доступ к speaker peripheral (первая найденная).
        env.speaker = setmetatable({}, { __index = function(_, k)
    ...
    local function safeUse(modulePath)
        ...
        local SAFE_PREFIXES = { "ui/", "util/", "audio/" }
